...
SSH_CMD="autossh -M 0"
SSH_CMD_MON="autossh -M "

case $command in
        start)  echo "#----`date '+%Y-%m-%d'` --- starting the proxy"
                ${SSH_CMD_MON} 11080 -qTfnN -D ${local_proxy_server_ip}:${port_number} ${proxy_host}
        ;;
        ...
        test)  echo "#----`date '+%Y-%m-%d'` --- test proxy connection"
                 ${SSH_CMD_MON} 11080 -vvvTN -D ${local_proxy_server_ip}:${port_number} ${proxy_host}
